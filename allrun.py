...
for run in runs:
    ...
    if run_id in average_accuracies and run_id in dirty_clean_accuracy:
        ...
        train_eps = run.config.get("train_numsteps")
        color = train_numsteps_colors.get(train_eps, 'gray')  # 如果 train_eps 不在 eps_colors 中则用灰色
...
for optimizer, color in train_numsteps_colors.items():
    indices = [i for i in range(len(colors)) if colors[i] == color]
    ax.scatter(x_values[indices], y_values[indices], color=color, marker='o', s=100, label=f'train_numsteps={optimizer}')
